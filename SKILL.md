...
- **Desktop-first**: finish `desktop` first; `tablet`/`mobile` only if the user wants responsive. Breakpoints are override slots; runtime cascades per-property mobile→tablet→desktop. Tablet writes affect what mobile inherits — [gui-desktop-first.md](rules/gui-desktop-first.md).
- **Batch writes (critical)**: use the fewest possible calls and batch by operation type (`add_entities` for creates, `remove_entities` for removals, `update_entities` with one `updates` array for property patches). Fragmented multi-call write sequences are a common source of editor instability/failures — [batching-and-verification.md](rules/batching-and-verification.md).
- **Physics prerequisite (required)**: if a task adds/updates rigid bodies, colliders, or any entity `physics` block, ensure the target scene has `physics.enabled: true` (and set gravity intentionally) via a batched `update_scene` call before validating behavior. Entity-level physics will not simulate when scene physics is off.
- **GUI — do not skim [gui-properties.md](references/entities/gui/gui-properties.md)**: for every `GUI_*` in the change set, read **[type defaults](references/entities/gui/gui-properties.md#type-defaults)** and field tables for every property you set or assume. MCP deep-merges `gui.currentValue`; unset keys still render via `GUI2D_*` JSX fallbacks — not in JSON/`get_entity`. Viewport ≠ JSON.
  - **Examples**: “Button” = `GUI_CONTAINER` + `GUI_TEXT`; default `overflow: scroll` → spurious scrollbars unless `visible`. Default container **150×150** → surprise width if you only set height.
- **Schema-safe values** (`SceneTypes`, `EntityTypes`, materials, geometry, GUI): only values allowed in [gui-properties.md](references/entities/gui/gui-properties.md); stray CSS keywords (`none`, `inherit`, …) can crash Yoga/uikit.
- **Non-GUI (3D) create path**: `add_entities` **deep-merges** `geometry` / `material` / `light` / `skybox` / `position` / `rotation` / `scale` (and similar animation tracks) with MCP defaults; **partial** `geometry` no longer wipes sibling keys. **Geometry scalars** are **normalized** on create; `update_entities` on `geometry.currentValue` is normalized the same way. See [primitives-common.md](references/entities/primitives/primitives-common.md#mcp-defaults-and-merge-behavior-non-gui), [lights-common.md](references/entities/lights/lights-common.md#mcp-defaults-create-path), [environment-common.md](references/entities/environment/environment-common.md#mcp-defaults-create-path), and [Non-GUI entities on create](rules/batching-and-verification.md#non-gui-entities-on-create).
...
| [references/entities/primitives/primitives-common.md](references/entities/primitives/primitives-common.md) | **Open before any `PRIMITIVE_*` create/edit.** Field rules, **MCP defaults + merge + hybrid scale** (see [MCP defaults](references/entities/primitives/primitives-common.md#mcp-defaults-and-merge-behavior-non-gui)). |
| [references/entities/lights/lights-common.md](references/entities/lights/lights-common.md) | **Open before any light create/edit.** `*_LIGHT` fields and **MCP create defaults** (see [MCP defaults](references/entities/lights/lights-common.md#mcp-defaults-create-path)). |
...
| [references/entities/environment/environment-common.md](references/entities/environment/environment-common.md) | **Open before `SKYBOX`/`HDR` work.** Environment fields and **MCP create defaults** (see [MCP defaults](references/entities/environment/environment-common.md#mcp-defaults-create-path)). |
